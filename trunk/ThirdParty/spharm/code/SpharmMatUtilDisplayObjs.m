...
for i = 1:numSbj
    ...
    h = figure('Name', ['Object: ' name],'NumberTitle', 'off');
    ...
    switch strtrim(char(confs.Export))
        ...
        case 'png'
            % save as png 
            pa_png = fullfile(path,'PNG');
            if ~exist(pa_png,'dir')
                mkdir(pa_png);
            end
            saveas(h,fullfile(pa_png,[name '_' suffix]),'png'); close(h);
        case 'both'
            % do both
            pa_png = fullfile(path,'PNG');
            if ~exist(pa_png,'dir')
            ...
            saveas(h,fullfile(pa_png,[name '_' suffix]),'png');            
    end
