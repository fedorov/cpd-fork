...
for i = 1:numSbj
    ...
    diary(fullfile([confs.OutDirectory '/Logs'],[date, '_', num2str(round(cputime)), '_' name '_scaleObjs.log']));
    
    sIDX = find(strcmpi([name ext], objNames)==1);
    ...
    if ~isempty(sIDX)
        ...
        new_name = [outDir '/' name(1:end-3) 'scl_' name(end-2:end) ext];
