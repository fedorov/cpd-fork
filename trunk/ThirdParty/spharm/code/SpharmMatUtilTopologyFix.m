...
for i = 1:numSbj
    ...
    switch method
        case 'InHouse_Fix'
            diary(fullfile([confs.OutDirectory '/Logs'],[date, '_', num2str(round(cputime)), '_' name '_fix_InHouse.log']));
            [bim_new,origin_new,vxsize_new,outNames{end+1}] = fix_bad_topology(file, confs);
        case 'PDM_Fix'
            diary(fullfile([confs.OutDirectory '/Logs'],[date, '_', num2str(round(cputime)), '_' name '_fix_PDM.log']));            
            optionSTR = '';
            ...
            new_name = sprintf('%s/%s_fix%s',path,name,ext);
            ...
            if ispc
                new_name = sprintf('tmp/%s_fix%s',name,ext);
                ...
                copyfile(file,wdir);
                [path2,fname,ext] = fileparts(file);
            ...
            outNames{end+1} = [confs.OutDirectory '/' name '_fix' ext];
    end
    diary('off');
    waitbar(i/numSbj)
end
