...
%while (iter<max_it)  && (sigma2 > 1e-8) %(sigma2 > 1e-8)
    ...
    if (fgt==0)  % no FGT
        [P1,Pt1, PX, L]=cpd_P_matlab(X,T, sigma2 ,outliers); st='';
    else         % FGT
    ...
    if viz, cpd_plot_iter(X, T, 0, sqrt(sigma2)); end;
%     A(:,iter+1)=getframe(fig1,winsize); 
...
%         end
%         im = getframe;
%         imwrite(im.cdata, ['cpd_result' num2str(iter) '.png'], 'png'); 

    iter=iter+1;

end

% mpgwrite(A,jet,'movie_template_to_subj.mpg');
