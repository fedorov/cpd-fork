function [TY, R, t, s, V, sigma2, P] = cpd_ssm_fem(X, SSM, w, errtol, maxiters, R, t, s, nMods, b, sigma2, mu, beta, E, nu, FV)
...
while ((iters < maxiters) && (err > errtol))
    ...
    LHS = s*s*Psi'*dP1*Psi + mu*sigma2*Lambda;
    RHS = -s*(P*X)*R;
    RHS = -Psi'*(reshape(RHS',[],1)+dP1*(s*s*(z_vec+v_vec)+s*repmat(R'*t,M,1)));
    b = LHS\RHS;
    ...
    disp(['Error fraction:', num2str(err)]);
        ...
        axis([-3,3,-3,3,-3,3]);
        legend({'target','ssm','fem'},'location','NEo')
        view(az, el);
        drawnow;
