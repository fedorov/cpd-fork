classdef fem_model < handle
    ...
    methods
        ...
        function [idx, elem, N] = findContainingElement(this, x, tol)
        ...
        function [idx, elem, N, in] = findNearestElement(this, x, tol)
            % Finds the element that contains a point, or the neasest if
            % outside
            %
            % [idx, elem, N] = findNearestElement(model, x)
            %   Finds the element that contains the point x, if indeed the
                ...
                if (this.smesh_tree ~= 0)
                    ...
                    for i=1:numOutside
                        ...
                        if (numel(eidxs) > 0)
                            ...
                            idx(oidx) = eidx;
                            elem{oidx} = e;
                            N{oidx} = getN(e, xem(1), xem(2), xem(3)); 
                        else
        ...
        function [idx, elem, N] = findContainingElementSlow(this, x)
            % Finds the element that contains a point
            %
            % [idx, elem] = findContainingElement(model, x)
            %   Finds the element that contains the point x, if indeed the
            ...
            for n=1:K
                for i=1:M
                    e = this.elems{i};
                    X = this.nodes(getNodeIdxs(e),:);
                    [inside, xem] = isInside(e, X, x(n,:));
                   
                    if (inside == 1)
                        idx(n) = i;
                        elem{n} = e;
                        N{n} = getN(e, xem(1), xem(2), xem(3));
                        break;
                    end
                end
            end
        ...
        function [ P, in ] = getInterpolationMatrix( this, X )
            ...
            [~, elem, N, in] = findNearestElement(this, X);
            ...
            P = sparse(r,c,s,M,getNumNodes(this));
        end
