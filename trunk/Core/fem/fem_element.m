classdef fem_element < handle
    ...
    methods(Abstract)
        ...
        %      coordinates.
        getdN(this,xi,eta,mu);
        
        % Determins whether natural coordinates fall inside the element
        %
        % [out] = isNaturalInside(elem, xem)
        %   Returns 1 if supplied natural coordinates [xi, eta, mu]
        %   lie inside the boundaries of this element
        isNaturalInside(this, xem);
    ...
    methods(Static)
        function elem = create(nodeIdxs)
            ...
            switch(size(nodeIdxs,2))
                case 4
                    elem = fem_tet_element(nodeIdxs);
                case 5
                    elem = fem_pyramid_element(nodeIdxs);
                case 6
                    elem = fem_wedge_element(nodeIdxs);
                case 8
                    elem = fem_hex_element(nodeIdxs);
                otherwise
    ...
    methods
        ...
        function [ x ] = getSpatialCoords(this, X, xi, eta, mu)
            ...
            %   Computes the spatial coordinates of a point inside the supplied
            %   element
            %
