...
for i = find(~emptyMask)
    ...
    for ptNo = find(qPtHitsTriangles(:))'
        ...
        IN(outPxIndsViaUnqIndiceMask(allqPtInds(ptNo), closestFacetDists<options.tol)) = true;
    end
end
...
function normalsAreUnified = checkNormalUnification(f)
% Helper function tests if any two faces sharing an edge label that edge's
% vertices in asc/desc order (good) rather than asc/asc or desc/desc. An
% edge shared by or more faces is deemed bad.
fromCols = 1:size(f,2);
edges = cat(3, f(:,fromCols), f(:,circshift(fromCols,[0 -1])));
...
unqEdgesFlat = unique([edgesFlat edgeAscOrderMask(:)],'rows');
normalsAreUnified = size(unqEdgesFlat,1) == size(edgesFlat,1);
